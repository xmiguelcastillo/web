    ...
    <div className="flex flex-col items-center  h-full">
      ...
      <div className="relative p-6 mt-6 rounded-lg shadow-lg w-[500px] max-w-4xl bg-indigo-800">
        ...
        <button
          ...
          className="absolute top-2  right-2 bg-blue-600 text-white text-xs px-4 py-2 rounded-md hover:bg-blue-700 transition"
        ...
        <pre className="bg-gray-800 text-white text-xs p-4 rounded-md overflow-x-auto">
          <code ref={codeRef} className="  {color: blue} language-javascript">
            {`import numpy as np
import random
...
def isDeadEnd(y, x, visited):
    for i in range(-1, 1):  # i in -1,0,1
        for j in range(-1, 1):  # i in -1,0,1
            if not (
                i == 0 and j == 0
            ):  # as if i==0 and j==0 then we are in teh same cell
...
def generateMazes(mazesNumber, num_rows, num_cols):
    ...
    for mazeInd in range(0, mazesNumber):
        ...
        while len(visited) < num_cols * num_rows:  # Repeat till visit all cells
            ...
            isDead = False
            while (
                (not isValidRow(crnt_row_index))
                or (not isValidCol(crnt_col_index))
            ...
            if rand_num < 0.3 and not isDead:
                ...
                print(
                    "Loc[" + str(crnt_row_index) + "],[" + str(crnt_col_index) + "] = 0"
                )
                ...
                if not isDead:
                    ...
                    isDead, unvisitRow, unvisitCol = isDeadEnd(
                        row_index, col_index, visited
                    )
                if isDead == True:  # if no unvisited neighbour
                    ...
                    if len(stack) > 0:
                        ...
                        if len(visited) < num_cols * num_rows:
                            while (
                                (not isValidRow(row_index))
                                or (not isValidCol(col_index))
...
def nextNeighbor(cellNumber, length):
    neighborList = [
        (cellNumber[0], cellNumber[1] + 1),
        (cellNumber[0], cellNumber[1] - 1),
        (cellNumber[0] + 1, cellNumber[1]),
        (cellNumber[0] - 1, cellNumber[1]),
    ]
    temp = []
...
def adaptedAStar(mLength, bList, start, goal, show=False):
    ...
    while currCell != gCell:
        ...
        if show:
            if currCell == start:
                plt.ion()
                im, fig, oLine, pLine = showLaunch(
                    mLength, map=magMap, pList=currPath, old=currTrack
                )
                plt.show()
            else:
                ...
                if magMap[2][newCell] != 2:
                    ...
                    if magMap[1][newCell] > magMap[1][coloredCell] + 1:
                        ...
                        bh.insert(
                            ...
                            * (
                                magMap[1][newCell]
                                + ManhattanDistanes(newCell, currCell)
                            )
                            - magMap[1][newCell],
                            newCell,
                        )
                    ...
                    if magMap[1][newCell] > magMap[1][coloredCell] + 1:
                        ...
                        bh.insert(
                            ...
                            (
                                ...
                                * (
                                    magMap[1][newCell]
                                    + ManhattanDistanes(newCell, gCell)
                                )
                                - magMap[1][newCell]
                            ),
                            newCell,
                ...
                if magMap[2][newCell] != 2:
                    ...
                    if magMap[1][newCell] > magMap[1][coloredCell] + 1:
                        ...
                        bh.insert(
                            oList,
                            oDict,
                            (magMap[1][newCell] + ManhattanDistanes(newCell, gCell)),
                            newCell,
                        )
        if not oList:
            return None, None
                ...
                if magMap[2][newCell] != 2:
                    ...
                    if magMap[1][newCell] > magMap[1][coloredCell] + 1:
                        ...
                        bh.insert(
                            oList,
                            oDict,
                            magMap[1][newCell] + ManhattanDistanes(newCell, currCell),
                            newCell,
                        )
        if not oList:
            return None, None
